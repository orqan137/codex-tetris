...
async function mcpSmokeTest() {
  ...
  try {
    ...
    if (opened.structuredContent?.session?.label !== "Smoke Codex session" || opened._meta?.["ui.resourceUri"] !== "ui://goal-tetris/board.v7.html") throw new Error("MCP native panel attach failed");
    const resource = await mcpRequest(server, { jsonrpc: "2.0", id: 4, method: "resources/read", params: { uri: "ui://goal-tetris/board.v7.html" } });
    if (!resource.contents?.[0]?.text?.includes("Goal Tetris") || !resource.contents[0].text.includes("arcade-theme") || !resource.contents[0].text.includes("completion-line") || !resource.contents[0].text.includes("confirm-clear") || !resource.contents[0].text.includes("goal_tetris_acknowledge") || !resource.contents[0].text.includes("data-view=\"history\"") || resource.contents[0].text.includes("localhost:4173")) throw new Error("Native widget resource is incomplete");
